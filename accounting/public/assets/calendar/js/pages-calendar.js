var Calendar = function () {
  ...
  defaultRange.event_start = moment();
  defaultRange.event_end = moment().add('days', 1);
  //Calendar
    ...
    var y = date.getFullYear();
      ...
      // print_r({{{getUpdates()}}});
        ...
        var update=Session.get('updates');
            ...
            console.log(new Date('2015-02-21'));
            // console.log(  update[2].update_classname[0].category_color );
              for (var i = 0; i <update.length; i++) {
                      u.push({
                        title : update[i].update_title,
                                                  start : new Date(update[i].update_start ),
                                                  end :  new Date(update[i].update_end ),
    ...
    -----------------------------------------------------------------*/
        ...
        $('#full-calendar').fullCalendar({
            ...
            select: function (start, end, allDay) {
              defaultRange.event_start = moment(start);
                        defaultRange.event_end = moment(start).add('hours', 1);
                        $.subview({
  ...
  var editFullEvent = function(el) {
    ...
    if ( typeof el == "undefined") {
      ...
      $(".form-full-event .event-start-date").val(defaultRange.event_start);
      $(".form-full-event .event-end-date").val(defaultRange.event_end);

      $('.form-full-event .no-all-day-range .event-range-date').val(moment(defaultRange.event_start).format('MM/DD/YYYY h:mm A') + ' - ' + moment(defaultRange.event_end).format('MM/DD/YYYY h:mm A'))
      .daterangepicker({
        startDate: defaultRange.event_start,
        endDate: defaultRange.event_end,
        timePicker: true,
        timePickerIncrement: 30,
        format: 'MM/DD/YYYY h:mm A'
      });

      $('.form-full-event .all-day-range .event-range-date').val(moment(defaultRange.event_start).format('MM/DD/YYYY') + ' - ' + moment(defaultRange.event_end).format('MM/DD/YYYY'))
      .daterangepicker({
        startDate: defaultRange.event_start,
        endDate: defaultRange.event_end
      });
